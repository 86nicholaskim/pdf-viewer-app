...
### 1) PNG/JPG/WebP 스트링 방식 (DataURL / Base64)
*   **패턴**: **값(Value) 전달 방식**
*   **특징**: 이진 데이터를 텍스트 문자열로 인코딩하여 직접 전달.
*   **성능 분석 (Trace 결과)**:
    *   **메모리**: JS Heap 메모리에 거대한 문자열이 직접 할당되어 점유율이 매우 높음 (**PNG > JPG > WebP**).
...
| **이메일 본문/팩스 발송** | **WebP (Base64)** | **값 (Value)** | **최고의 압축률**: JPG보다 작으면서 투명도 지원, HTML 내 데이터 내장 시 유리. |
| **고화질 아카이빙** | **PNG (Base64)** | **값 (Value)** | 손실 없는 화질로 문서를 영구 보관해야 할 때 사용. (단, 파일 용량이 매우 커짐) |
| **웹드라이버 스크래핑** | **WebP (Base64)** | **값 (Value)** | 주소가 아닌 **데이터 자체를 캡처**해야 전송 시 이미지가 유실되지 않음. |
| **오프라인 파일 저장** | **WebP (Base64)** | **값 (Value)** | 단일 HTML 파일만으로 언제 어디서든 고화질 문서를 볼 수 있게 함. |

### 💡 왜 섞어 써야 하나요?
1.  **내부 작업 (Internal)**: 내 브라우저 메모리 안에서 끝나는 일은 **Blob**이 압도적으로 빠르고 가볍습니다.
2.  **외부 전송 (External)**: 데이터가 브라우저 밖으로 나가는 순간 `blob:` 주소는 무용지물이 됩니다. 이때는 무겁더라도 데이터 자체가 박혀 있는 **Base64**가 필수이며, 최적의 효율을 위해 **WebP** 또는 호환성을 위한 **JPG**를 선택합니다.
...
| :--- | :---: | :--- | :--- |
| **A4 비즈니스 인쇄** | **1.5** | **WebP (0.8)** | **차세대 표준**: JPG보다 30% 더 작으면서도 고화질 유지. |
| **고화질 아카이빙** | **2.0 이상** | **PNG** | **무손실 보관**: 텍스트와 그래픽의 단 1% 손실도 허용하지 않는 영구 보관용. |
| **이메일/모바일 전송** | **1.2** | **WebP (0.6)** | **전송 안정성**: 모바일 환경에서 즉시 로딩되도록 문자열 길이를 최소화. |
| **팩스(Fax) 전용** | **1.0** | **JPG (0.5)** | **저속망 최적화**: 팩스 규격(200 DPI 이하) 및 구형 시스템 호환성 고려. |
| **화면 썸네일** | **0.3 ~ 0.5** | **WebP (0.4)** | **메모리 절약**: 형체만 확인하는 용도로 JS Heap 부하를 최소화. |
...
2.  **포맷(Format)과 문자열 길이**:
    ...
    *   **WebP(0.8)**는 PNG 대비 문자열 길이를 **70% 이상 단축**시키며, JPG보다도 더 효율적입니다.

3.  **사용자 경험(UX) 균형**:
    *   "본 것과 뽑은 것이 같다"는 신뢰를 주기 위해 **Scale 1.5 + WebP 0.8** 조합을 비즈니스 표준으로 권장합니다.
...
*   **외부용 (공유/전송)**: **Scale 1.5 + WebP(0.8) Base64**를 기본으로 하되, 대량 전송 시 **Scale 1.2**로의 조절을 고려하십시오.
