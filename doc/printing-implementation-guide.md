...
2.  **외부 전송 (External)**: 데이터가 브라우저 밖으로 나가는 순간 `blob:` 주소는 무용지물이 됩니다. 이때는 무겁더라도 데이터 자체가 박혀 있는 **Base64**가 필수이며, 전송 효율을 위해 **JPG 압축**이 정석입니다.

---

## 6. 최종 결론 및 권장 구현 방향

1.  **성능 최적화**: 인쇄 팝업 로직은 **Blob** 방식을 기본으로 채택하여 안정성을 확보하십시오.
2.  **호환성 확보**: 외부 시스템(Email, WebDriver, API 전송) 연동 시에는 **JPG 0.7~0.8 Base64** 형식을 사용하여 데이터 독립성과 전송 성공률을 높이십시오.
3.  **관리 책임**: Blob 사용 시에는 `URL.revokeObjectURL()`을 통한 메모리 반납을, Base64 사용 시에는 데이터 크기에 의한 전체 시스템 부하를 항상 고려하십시오.
